...
gap_px    = st.sidebar.slider("Espacio entre activos (px)", 10, 40, 13)
height_px = st.sidebar.slider("Altura barra (px)", 36, 72, 58)

# Panel/logo que desfila DENTRO de la cinta
logo_path_input = st.sidebar.text_input("Logo (ruta local o URL)", "LOGO1.png")
panel_w  = st.sidebar.slider("Ancho del panel de logo en la cinta (px)", 220, 800, 220)
panel_h  = st.sidebar.slider("Alto del logo dentro del panel (px)", 28, 120, 120)
every_n  = st.sidebar.slider("Insertar el logo cada N activos", 3, 12, 8)

# Logo fijo a la derecha + reserva (activado por defecto)
use_fixed_logo = st.sidebar.checkbox("Logo fijo a la derecha (además del logo en la cinta)", True)
fixed_logo_h   = st.sidebar.slider("Alto logo fijo (px)", 20, 120, 120, disabled=not use_fixed_logo)
right_reserve  = st.sidebar.slider("Reserva derecha (px) si logo fijo", 160, 360, 329, disabled=not use_fixed_logo)

# Tema y overlay (ambos activos por defecto)
dark     = st.sidebar.checkbox("Tema oscuro (preview)", True)
overlay  = st.sidebar.checkbox("Overlay OBS (fondo transparente)", True)

# Reloj grande
clock_px = st.sidebar.slider("Tamaño del reloj (px)", 14, 28, 28)

# Autorefresco de datos (0 = desactivado)
